 ...
 */
public class SplitExpPulseAsymMig extends Demography {
    ...
    String ParamsToString(double[] params) {
        return ("CurrentPop1: " + params[0] + "\tpop1Ancestral: " + params[1] + "\tpop2Current: " + params[2]
                ...
                + "\tm12: " + params[6] + "\tm21: " + params[7] + "\tGa: " + params[8]);
    }

    double expectedFraction(double[] params, double L1, double L2) {

        //NOTE: quite inefficient code. TODO: improve
        ...
        double m12 = params[6];
        double m21 = params[7];
        double Ga = params[8]; // time of pulse
        double popSelector = params[9]; // 1 if both in 1, 2 if both in 2, in two pops otherwise
...
//                System.out.println(i + "\t" + PopSize[i]);
            M_curr[0][0] = M_curr[0][0] * M_now[0][0] + M_curr[0][1] * M_now[1][0];
            M_curr[0][1] = M_curr[0][0] * M_now[0][1] + M_curr[0][1] * M_now[1][1];
            M_curr[1][0] = M_curr[1][0] * M_now[0][0] + M_curr[1][1] * M_now[1][0];
            M_curr[1][1] = M_curr[1][0] * M_now[0][1] + M_curr[1][1] * M_now[1][1];

            if (false && popSelector != 1 && popSelector != 2 && L1 == 1.0) {
                System.out.println(i + "\t" + M_curr[0][0] + " " + M_curr[0][1] + " " + M_curr[1][0] + " " + M_curr[1][1]);
            }
...
//            if (popSelector==1.0&&L1==1&&L2==2) System.out.println("\n" + i + " " + CP[i] + " " + Pop1Size[i] + " " + Pop2Size[i]);
        ...
        for (int i = 2; i <= G + 1; i++) {
            NCG[i] = NCG[i - 1] * NC[i];
            if (false && popSelector != 1.0 && popSelector != 2.0 && L1 == 1 && L2 == 2) {
                System.out.println(i + "\t" + NCG[i] + "\t" + CP[i]);
            }
        }
        ...
        for (int k = 1; k <= G; k++) {
            ...
            p1 += NCG[k] * CP[k] * eL;
//            if (popSelector != 1 && popSelector != 2 && L1 == 1.0) {
//                System.out.println(k + "\t" + (NCG[k] * CP[k] * eL));
//            }
            if (false && popSelector != 1.0 && popSelector != 2.0 && L1 == 1 && L2 == 2) {
                System.out.println(k + "\t" + NCG[k] + "\t" + CP[k] + "\t" + eL + "\t" + NCG[k] * CP[k] * eL + "\t" + p1);
            }
        }
